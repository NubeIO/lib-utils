...
func ContainsIgnoreCase(str string, args ...string) bool {
	for _, s := range args {
		if strings.Contains(strings.ToUpper(str), strings.ToUpper(s)) {
			return true
		}
	}
	return false
...
// InPrefix str prefix matches with one of args
func InPrefix(str string, args ...string) bool {
	for _, s := range args {
...
func ContainsNil(str ...*string) bool {
	for _, s := range str {
		if s == nil {
			return true
		}
